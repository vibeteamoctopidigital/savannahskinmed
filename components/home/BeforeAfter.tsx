...
//             {/* Fixed bottom-center — no longer tracks the slider position */}
    ...
    <section className={`section ${bg || 'bg-white'} px-4 pt-4 sm:px-6 lg:px-8 lg:pt-8`}>
        ...
        >
          ...
          <div
            ...
            className="relative mx-auto aspect-[838/500] w-full max-w-[806px] cursor-ew-resize touch-none select-none overflow-hidden rounded-xl"
          >
            {/* Untreated state */}
            <Image
              key={`before-${slide.before}`}
  ...
  <span className="pointer-events-none absolute top-[6%] sm:top-auto sm:bottom-[6%] right-[6%] z-10 font-serif text-[32px] text-white text-shadow-hero">
    After
            ...
            <input
              ...
              onChange={(e) => setPos(Number(e.target.value))}
              aria-label={`Reveal the after image for ${slide.procedure}`}
              className="absolute bottom-2 left-1/2 z-30 h-8 w-[60%] -translate-x-1/2 cursor-ew-resize touch-none opacity-0"
            />
          </div>
